...
fn main() {
    ...
    #[cfg(windows)]
    println!("cargo:rerun-if-changed=src\\bin\\d4v2.bin");
    #[cfg(unix)]
    println!("cargo:rerun-if-changed=src/bin/d4v2.bin");
    ...
    if !Path::new("d4v2/build/d4").exists() {
        ...
        Command::new("mkdir")
            .arg("d4v2")
            .output()
            .expect("wasn't able to create the 'd4v2' folder");
        Command::new("git")
            .args([
                "clone",
                "https://github.com/SoftVarE-Group/d4v2.git",
                "d4v2/.",
            ])
            .output()
            .expect("could not clone the d4v2 repository");

        // build the d4v2 binary
        let current_dir = &env::current_dir().unwrap();
        env::set_current_dir(
            current_dir.canonicalize().unwrap().join("d4v2").as_path(),
        )
        .unwrap();
        match Command::new("./build.sh").output() {
            Ok(_) => (),
            Err(err) => eprintln!(
                "An error occured while trying to build the d4v2 binary: {err}"
            ),
        }
    ...
    out.write_all(&std::fs::read("d4v2/build/d4").unwrap())
        .unwrap();
}
