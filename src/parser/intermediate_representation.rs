    ...
    pub fn apply_clause(
        ...
        match self
            ...
            Some(((clause, application), replacement)) => {
                ...
                } else if application == ClauseApplication::Remove {
        ...
        if clause.len() == 1 && application == ClauseApplication::Add {
        ...
        println!(
            "For clause: {:?}, Replace CNF clauses: {}",
            clause,
            cnf.len()
        );
        if DEBUG {
            //println!("CNF clauses: {:?}", cnf);
        }
...
mod test {
    ...
    use crate::{
        parser::{
            ...
            intermediate_representation::{ClauseApplication, DEBUG, MERMAID_DEBUG},
            persisting::write_as_mermaid_md,
        },
        Ddnnf, IncrementalStrategy,
    };
    ...
    fn check_for_cardinality_correctness(path: &str, break_point: usize) {
        ...
        let mut card_of_features_w: Vec<rug::Integer> = Vec::new();
    ...
    fn undo_clause_remove() {
        ...
        for path in ddnnf_file_paths {
            ...
            let mut expected_card_of_features: Vec<rug::Integer> = Vec::new();
    ...
    fn adding_new_features() {
        ...
        ddnnf
            .inter_graph
            .apply_clause(vec![6, -6], ClauseApplication::Add);
        ddnnf.rebuild();
        ...
        ddnnf
            .inter_graph
            .apply_clause(vec![-5, -6], ClauseApplication::Add);
        ddnnf.rebuild();
        assert_eq!(6, ddnnf.number_of_variables);
    ...
    fn adding_unit_clause() {
        ...
        ddnnf_missing_clause2
            .inter_graph
            .apply_clause(vec![1], ClauseApplication::Add); // indirectly via adding a clause
