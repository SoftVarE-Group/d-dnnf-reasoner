use nom::{
    ...
    combinator::{map_res, value, recognize},
    IResult
};
...
/// assert_eq!(lex_line(and_str).unwrap().1.1, vec![3 as usize, 1 as usize, 2 as usize]);
/// 
/// let header_str = "nnf 32 13 23"; 
/// assert_eq!(lex_header(header_str).unwrap().1.0, Header);
/// assert_eq!(lex_header(header_str).unwrap().1.1, vec![32 as usize, 13 as usize, 23 as usize]);
/// 
...
fn get_number1_wrapper(kind: TId) -> Box<dyn Fn(&str) -> Result<Token,&str>> {
    Box::new(move |s: &str| -> Result<Token,&str> {
        ...
        match (kind, r){
            (And, mut r) => {r.swap_remove(0); //swap_remove() has O(1) because the empty space gets filled with the last element
                            Ok((And, r))},
            (Or, mut r) => {r.remove(1);
                            Ok((Or, r))},
            (Header, r) => Ok((Header, r)),
            _ => panic!("Tried to parse an alternating sequence after NodeType {:?}", kind)
        }
    })
}
...
mod test {
    ...
    fn test_lex_lines() {
        ...
        assert_eq!(lex_line(or_str).unwrap().1.1, vec![44 as usize, 40 as usize]);

        assert_eq!(lex_line(positive_literal_str).unwrap().1.0, PositiveLiteral);
        assert_eq!(lex_line(positive_literal_str).unwrap().1.1, vec![3 as usize]);
    }
    ...
    fn test_individual_lexer(){
        ...
        assert_eq!(lex_and(and_str).unwrap().1.1, vec![1 as usize]);

        let or_str = "O 10 2 20 24";
        assert_eq!(lex_or(or_str).unwrap().1.0, Or);
        assert_eq!(lex_or(or_str).unwrap().1.1, vec![24 as usize, 20 as usize]);

        let positive_literal_str = "L 13";
        assert_eq!(lex_positive_literal(positive_literal_str).unwrap().1.0, PositiveLiteral);
        assert_eq!(lex_positive_literal(positive_literal_str).unwrap().1.1, vec![13 as usize]);

        let negative_literal_str = "L -113";
        assert_eq!(lex_negative_literal(negative_literal_str).unwrap().1.0, NegativeLiteral);
        assert_eq!(lex_negative_literal(negative_literal_str).unwrap().1.1, vec![113 as usize]);
