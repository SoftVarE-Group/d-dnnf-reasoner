...
pub struct SamplingEvalResult {
    /// The amount of configs in the sample
    sample_size: usize,
...
pub fn eval_sample(
    ...
    t: usize,
) -> Result<SamplingEvalResult, String> {
    ...
    let n_selected_features_sum = sample
        .iter()
        .map(|config| {
            config
    ...
    let n_selected_features_var = sample
        .iter()
        .map(|config| {
            config
                .get_decided_literals()
        ...
        .map(|config_n_selected_features| {
            (config_n_selected_features as f64 - n_selected_features_avg).pow(2)
        })
        .sum::<f64>()
        / sample_size as f64;
    ...
    ) = eval_additional_interactions(sample, ext_ddnnf, t);

    Ok(SamplingEvalResult {
        sample_size,
        config_values_sum,
        config_values_avg,
        config_values_var,
...
pub fn eval_additional_interactions(
    sample: &Sample,
    ext_ddnnf: &ExtendedDdnnf,
    t: usize,
) -> (usize, f64, f64) {
    ...
    for config in sample.iter() {
        TInteractionIter::new(
            config.get_literals(),
            min(config.get_n_decided_literals(), t),
        )
    ...
    let additional_interactions_count =
        interaction_counter.values().sum::<usize>() - interaction_counter.len();
    let additional_interactions_value_sum = interaction_counter
        .iter()
        .map(|(interaction, count)| {
            ext_ddnnf.get_objective_fn_val_of_literals(&interaction[..]) * (count - 1) as f64
        })
        .sum();
    let additional_interactions_value_avg =
        additional_interactions_value_sum / additional_interactions_count as f64;
...
pub fn check_sample_validity(
    sample: &Sample,
    ext_ddnnf: &ExtendedDdnnf,
    t: usize,
) -> Result<(), String> {
    for literals in sample
        .iter()
    ...
    let all_literals = (-(ext_ddnnf.ddnnf.number_of_variables as i32)
        ..=ext_ddnnf.ddnnf.number_of_variables as i32)
        .filter(|&literal| literal != 0)
        .collect_vec();
...
mod test {
    ...
    fn compare_sampling() {
        ...
        let n_features = 998;
        let vp9: Ddnnf = build_ddnnf(
            Path::new("tests/data/busybox_1_28_0.dimacs.nnf"),
            Some(n_features),
        );
        // let n_features = 42;
        ...
        for t in 2..=2 {
            ...
            let now = Instant::now();
            let normal_sample_result = ext_ddnnf
                .ddnnf
                .sample_t_wise(t)
                .get_sample()
                .unwrap()
