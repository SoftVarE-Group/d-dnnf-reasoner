...
buildscript { dependencies { classpath("net.java.dev.jna:jna:5.+") } }
...
// When no library oder library folder is passed, we only target the current system and directly use
// Rust to build the library.
val onlyCurrent = !singleLibrary && !multipleLibraries

val resourcePrefix: String =
    if (hasProperty("generatePrefix")) {
...
tasks.processResources { dependsOn("nativeLibrary") }
...
if (onlyCurrent) {
  ...
  tasks.named("nativeLibrary") { dependsOn("buildRust") }
}
...
tasks.register<Exec>("generateBindings") {
  ...
  args(
      ...
      "kotlin",
      "--out-dir",
      "${layout.projectDirectory}/src/main/kotlin",
      "--library",
      "${libraryDest}/${libraryName}",
      "--metadata-no-deps",
      "--no-format",
  )
