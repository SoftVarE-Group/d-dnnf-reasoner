 ...
 * @param features How many features the corresponding model has. Can be `null` in which case this
 *   will be determined by building the d-DNNF.
 */
...
fun isSample(result: SamplingResult): Boolean =
    when (result) {
      is SamplingResult.ResultWithSample -> true
      else -> false
    }

fun isEmpty(result: SamplingResult): Boolean =
    when (result) {
      is SamplingResult.Empty -> true
      else -> false
    }

fun isVoid(result: SamplingResult): Boolean =
    when (result) {
